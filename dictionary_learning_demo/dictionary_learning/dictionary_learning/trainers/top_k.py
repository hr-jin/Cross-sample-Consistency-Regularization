...
class AutoEncoderTopK(Dictionary, nn.Module):
    ...
    def __init__(self, activation_dim: int, dict_size: int, k: int, dtype=t.float32, device=None):
        super().__init__()
        ...
        self.decoder = nn.Linear(dict_size, activation_dim, bias=False, device=device, dtype=dtype)
        self.decoder.weight.data = set_decoder_norm_to_unit_norm(
            self.decoder.weight, activation_dim, dict_size
        )

        self.encoder = nn.Linear(activation_dim, dict_size, device=device, dtype=dtype)
        self.encoder.weight.data = self.decoder.weight.T.clone()
        self.encoder.bias.data.zero_()

        self.b_dec = nn.Parameter(t.zeros(activation_dim, device=device, dtype=dtype))
...
class TopKTrainer(SAETrainer):
    ...
    def __init__(
        ...
        if device is None:
            ...
            self.device = device
        self.ae = dict_class(activation_dim, dict_size, k, dtype=dtype, device=self.device)
