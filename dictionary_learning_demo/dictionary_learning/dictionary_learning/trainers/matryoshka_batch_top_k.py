...
class MatryoshkaBatchTopKSAE(Dictionary, nn.Module):
    def __init__(
        self, activation_dim: int, dict_size: int, k: int, group_sizes: list[int], dtype=t.float32, device=None
    ):
        super().__init__()
        ...
        self.W_enc = nn.Parameter(t.empty(activation_dim, dict_size, dtype=dtype, device=device))
        self.b_enc = nn.Parameter(t.zeros(dict_size, dtype=dtype, device=device))
        self.W_dec = nn.Parameter(
            t.nn.init.kaiming_uniform_(t.empty(dict_size, activation_dim, dtype=dtype, device=device))
        )
        self.b_dec = nn.Parameter(t.zeros(activation_dim, dtype=dtype, device=device))
...
class MatryoshkaBatchTopKTrainer(SAETrainer):
    def __init__(
        ...
        wandb_name: str = "MatryoshkaBatchTopKSAE",
        ...
        self.ae = dict_class(activation_dim, dict_size, k, group_sizes, dtype=dtype, device=device)
