...
class Example:
    ...
    def to_str(self, mark_toks: bool = False) -> str:
        return (
            "".join(
                f"<<{tok}[act:{activation}]>>" if (mark_toks and is_active) else tok
                for tok, is_active, activation in zip(self.str_toks, self.toks_are_active, self.acts)
            )
            .replace("�", "")
            .replace("\n", "↵")
        )
...
class AutoInterp:
    ...
    def __init__(
        ...
        self.n_api_workers = n_api_workers
    ...
    async def run(
        ...
        with ThreadPoolExecutor(max_workers=self.n_api_workers) as executor:
            tasks = [
    ...
    async def run_single_feature(
        ...
        gen_prompts = self.get_generation_prompts(generation_examples, self.cfg.n_top_ex_for_generation)
        ...
        if api_result is None or api_result[0] is None:
            print(f"Skipping latent {latent} due to API failure in generation phase.")
            return None
        (explanation_raw,), logs, system_prompt, user_prompt = api_result
        ...
        results = {
            ...
            "explain_user_prompt": user_prompt,
        }
        ...
        if self.cfg.scoring:
            ...
            if api_result is None or api_result[0] is None:
                print(f"Skipping latent {latent} due to API failure in scoring phase.")
                return None
            (predictions_raw,), logs, system_prompt, user_prompt = api_result
            ...
            results |= {
                ...
                "predict_system_prompt": system_prompt,
                "predict_user_prompt": user_prompt,
            }
    ...
    def get_api_response(
        ...
        """Generic API usage function for OpenAI with 3 retries on failure."""
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    print(f"All retries failed for this API call. Skipping.")
        ...
        if not response:
            logs = f"API call failed after {max_retries} attempts."
            assert False, f"API call failed after {max_retries} attempts."
        ...
        return response, logs, system_prompt, user_prompt

    def get_generation_prompts(self, generation_examples: Examples, n_activates: int) -> Messages:
        assert len(generation_examples) > 0, "No generation examples found"

        activating_examples = generation_examples[:n_activates]
        non_activating_examples = generation_examples[n_activates:]

        activating_str = "\n".join(
            [
                f"ACTIVATE ({i + 1}). {ex.to_str(mark_toks=True)}"
                for i, ex in enumerate(activating_examples)
            ]
        )

        start_index_for_non_activating = len(activating_examples) + 1
        non_activating_str = "\n".join(
            [
                f"INACTIVATE ({i + start_index_for_non_activating}). {ex.to_str(mark_toks=False)}"
                for i, ex in enumerate(non_activating_examples)
            ]
        )

        examples_as_str = activating_str
        if non_activating_str:
            if activating_str:
                examples_as_str += "\n\n"
            examples_as_str += non_activating_str

        SYSTEM_PROMPT = """We're studying neurons in a neural network. Each neuron activates on some particular word/words/substring/concept in a short document. The activating words in each document are indicated with <<{{token}}[act:{{activation}}]>>.
We will give you a list of ACTIVATE documents (where the neuron fires, ordered by strength) and INACTIVATE documents (where the neuron does not fire). Look at the marked parts of the ACTIVATE documents, and consider the INACTIVATE documents to refine the activation boundary. Summarize in a single sentence what the neuron is activating on. Try not to be overly specific or overly broad. Your explanation should cover all activating words and not cover any inactive text. Pay attention to things like capitalization and punctuation if relevant. Keep the explanation as short and simple as possible, limited to 30 words or less. Omit punctuation and formatting."""
        ...
        USER_PROMPT = (
            f"""The relevant documents are given below:\n\n{examples_as_str}"""
        )
    ...
    def gather_data(self) -> tuple[dict[int, Examples], dict[int, Examples]]:
        ...
        for i, latent in tqdm(
            ...
            bottom_indices = get_zero_activation_sequences(
                acts[..., i],
                k=self.cfg.n_random_ex_for_scoring,
                buffer=self.cfg.buffer,
            )
            bottom_toks = index_with_buffer(
                self.tokenized_dataset, bottom_indices, buffer=self.cfg.buffer
            )

            if bottom_toks.numel() == 0 and self.cfg.n_random_ex_for_scoring > 0:
                 continue
            ...
            generation_examples[latent] = Examples(
                ...
                + create_examples(bottom_toks[:self.cfg.n_iw_sampled_ex_for_generation])
            )
            scoring_examples[latent] = Examples(
                create_examples(
                    top_toks[top_scoring_indices], top_values[top_scoring_indices]
                )
                + create_examples(bottom_toks),
                shuffle=True,
            )
...
def run_eval_single_sae(
    ...
    if os.path.exists(tokens_path):
        tokenized_dataset = torch.load(tokens_path, map_location=device)
    else:
...
def run_eval(
    ...
    for sae_release, sae_object_or_id in tqdm(
        ...
        sae_eval_result = run_eval_single_sae(
            config, sae, model, device, artifacts_folder, api_key, sparsity, base_url=base_url, openai_model=openai_model, n_api_workers=n_api_workers
        )
        ...
        all_scores = [r["score"] for r in sae_eval_result.values()]
