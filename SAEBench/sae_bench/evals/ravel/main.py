...
class LazyDataLoader:
    """
    A dataloader that lazily processes batches to save memory and startup time.
    """

    def __init__(self, formatted_pairs, model, eval_config):
        self.formatted_pairs = formatted_pairs
        self.model = model
        self.eval_config = eval_config
        self.batch_size = eval_config.llm_batch_size
        self.tokenizer = AutoTokenizer.from_pretrained(eval_config.model_name)
        self.num_batches = len(formatted_pairs) // self.batch_size

        self.max_base_len = max(len(pair[0]) for pair in formatted_pairs) if formatted_pairs else 0
        self.max_source_len = max(len(pair[1]) for pair in formatted_pairs) if formatted_pairs else 0
    ...
    def __iter__(self):
        for batch_idx in range(self.num_batches):
            ...
            batch_data = self.formatted_pairs[batch_start:batch_end]
            ...
            base_tokens_BL, base_attn_mask_BL = custom_left_padding(
                self.tokenizer, base_tokens_BL, self.max_base_len
            )
            source_tokens_BL, source_attn_mask_BL = custom_left_padding(
                self.tokenizer, source_tokens_BL, self.max_source_len
            )

            base_tokens_BL = base_tokens_BL.to(self.model.device)
            base_attn_mask_BL = base_attn_mask_BL.to(self.model.device)
            source_tokens_BL = source_tokens_BL.to(self.model.device)
            source_attn_mask_BL = source_attn_mask_BL.to(self.model.device)

            base_pos_B = torch.tensor(base_pos_B).to(self.model.device)
            source_pos_B = torch.tensor(source_pos_B).to(self.model.device)
            base_pred_B = torch.tensor(base_pred_B).to(self.model.device)
            source_pred_B = torch.tensor(source_pred_B).to(self.model.device)
...
def create_dataloader_from_pairs(formatted_pairs, model, eval_config):
    ...
    Returns:
        dataloader: LazyDataLoader instance
    """
    dataloader = LazyDataLoader(formatted_pairs, model, eval_config)
...
def run_eval_single_cause_attribute(
    ...
    iso_score, cause_score, badcases = intervention.generate_batched_interventions(
        model,
        trained_mdbm,
        tokenizer,
        val_loader,
...
def run_eval_single_dataset(
    ...
    for cause_attribute in attributes:
        ...
        if "badcases" in mdbm_results:
            for bc in mdbm_results["badcases"]:
                bc["cause_attribute"] = cause_attribute
                bc["entity_class"] = entity_class
            all_badcases.extend(mdbm_results["badcases"])
            del mdbm_results["badcases"]
    ...
    return results_dict, per_class_results_dict, all_badcases
...
def run_eval_single_sae(
...
) -> tuple[dict[str, float | dict[str, float]], dict, list[dict]]:
    ...
    return results_dict, per_class_dict, all_badcases
...
def run_eval(
    ...
    config.model_name = LLM_NAME_MAP.get(config.model_name, config.model_name)
    ...
    for sae_release, sae_object_or_id in tqdm(
        ...
        eval_results, per_class_dict, badcases = run_eval_single_sae(
            config,
            sae,
            model,
            device,
