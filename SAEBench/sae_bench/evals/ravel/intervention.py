...
def generate_batched_interventions(
    ...
    for batch in tqdm(val_loader, desc="Generating with interventions"):
        ...
        for base_text, base_label, generated_string, base_pred, source_pred in zip(
            ...
            is_correct = evaluate_completion(base_text, base_label, generated_string)
            if base_pred == source_pred:
                iso_scores.append(is_correct)
                if not is_correct:
                    badcases.append(
                        {
                            "type": "isolation",
                            ...
                            "generated_text": generated_string,
                        }
                    )
            else:
                cause_scores.append(is_correct)
                if not is_correct:
                    badcases.append(
                        {
                            "type": "cause",
    ...
    return iso_score, cause_score, badcases
