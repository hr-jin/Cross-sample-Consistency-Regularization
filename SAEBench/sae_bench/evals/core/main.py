...
def run_evals(
    ...
    if eval_config.compute_featurewise_weight_based_metrics:
        feature_density = feature_metrics.get("feature_density")
        feature_metrics |= get_featurewise_weight_based_metrics(sae, feature_density)
...
def calculate_max_cosine_sim(
    encoder_DF: torch.Tensor, batch_size: int = 100, alive_mask: torch.Tensor | None = None
) -> torch.Tensor:
    """
    ...
    alive_mask: Optional boolean tensor of shape (F,) indicating which features are alive.
                If provided, max cosine sim is calculated only against alive features.
                Dead features will have a value of -inf.
    ...
    for start in range(0, F_, batch_size):
        ...
        if alive_mask is not None:
            sims_CF[:, ~alive_mask] = float("-inf")

        row_max_sims_C = sims_CF.max(dim=1).values

        if alive_mask is not None:
            chunk_mask = alive_mask[start:end]
            row_max_sims_C[~chunk_mask] = float("-inf")
...
def get_featurewise_weight_based_metrics(sae: SAE, feature_density: list[float] | None = None) -> dict[str, Any]:
    ...
    elif sae.cfg.architecture != "gated" and sae.cfg.architecture != "co_gated":
    ...
    if feature_density is not None:
        alive_mask = torch.tensor(feature_density, device=sae.device) > 1e-7

        max_encoder_cosine_sim_alive = calculate_max_cosine_sim(sae.W_enc, alive_mask=alive_mask).cpu().tolist()
        max_decoder_cosine_sim_alive = calculate_max_cosine_sim(sae.W_dec.T, alive_mask=alive_mask).cpu().tolist()

        metrics["max_encoder_cosine_sim_alive"] = max_encoder_cosine_sim_alive
        metrics["max_decoder_cosine_sim_alive"] = max_decoder_cosine_sim_alive
    ...
    for _ in batch_iter:
        ...
        if compute_variance_metrics:
            ...
            mean_sum_of_squares.append(
                (flattened_sae_input).pow(2).sum(dim=-1).mean(dim=0)
            )
            mean_act_per_dimension.append(
                (flattened_sae_input).pow(2).mean(dim=0)
            )
...
def get_recons_loss(
    ...
    def kl(original_logits: torch.Tensor, new_logits: torch.Tensor):
        log_original_probs = torch.nn.functional.log_softmax(original_logits, dim=-1)
        log_new_probs = torch.nn.functional.log_softmax(new_logits, dim=-1)
        original_probs = torch.exp(log_original_probs)
        kl_div = original_probs * (log_original_probs - log_new_probs)
        kl_div[original_probs == 0] = 0
        ...
        for i in range(num_features):
            feature_metrics_by_feature.append(
                CoreFeatureMetric(
                    ...
                    max_decoder_cosine_sim_alive=round(
                        flattened_feature_metrics.get("max_decoder_cosine_sim_alive", [-1.0] * num_features)[i],
                        DEFAULT_FLOAT_PRECISION,
                    ),
                    max_encoder_cosine_sim_alive=round(
                        flattened_feature_metrics.get("max_encoder_cosine_sim_alive", [-1.0] * num_features)[i],
                        DEFAULT_FLOAT_PRECISION,
                    ),
                )
            )
...
def calculate_misc_metrics(feature_metrics: dict[str, torch.Tensor]) -> dict:
    ...
    if "max_encoder_cosine_sim_alive" in feature_metrics:
        feature_densities_F = torch.Tensor(feature_metrics["feature_density"])
        alive_mask = feature_densities_F > 1e-7

        max_enc_alive = torch.Tensor(feature_metrics["max_encoder_cosine_sim_alive"])
        max_dec_alive = torch.Tensor(feature_metrics["max_decoder_cosine_sim_alive"])

        if alive_mask.sum() > 0:
            avg_max_enc_alive = max_enc_alive[alive_mask].mean().item()
            avg_max_dec_alive = max_dec_alive[alive_mask].mean().item()
        else:
            avg_max_enc_alive = 0.0
            avg_max_dec_alive = 0.0

        misc_metrics["average_max_encoder_cosine_sim_alive"] = avg_max_enc_alive
        misc_metrics["average_max_decoder_cosine_sim_alive"] = avg_max_dec_alive
...
def multiple_evals(
    ...
    for sae_release, sae_object_or_id in tqdm(
        ...
        if sae.cfg.context_size is None:
            sae.cfg.context_size = context_size
        sae.cfg.__post_init__()
        ...
        scalar_metrics, feature_metrics = run_evals(
            ...
            ignore_tokens={
                current_model.tokenizer.pad_token_id,
                current_model.tokenizer.eos_token_id,
                current_model.tokenizer.bos_token_id,
            },
            verbose=verbose,
        )
...
if __name__ == "__main__":
    ...
    print("Evaluation complete. All results have been saved incrementally.")
