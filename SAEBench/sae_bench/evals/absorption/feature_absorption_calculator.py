...
class FeatureAbsorptionCalculator:
    ...
    def calculate_absorption(
        ...
        for batch_prompts in batchify(prompts, batch_size=self.batch_size):
            ...
            if hasattr(sae, "fold_norm_scaling_factor") and sae.fold_norm_scaling_factor is not None:
                batch_acts = batch_acts * sae.fold_norm_scaling_factor
            ...
            for i, prompt in enumerate(tqdm(batch_prompts, disable=not show_progress)):
                ...
                act_probe_proj = batch_probe_projections[i].item()
                sae_act_probe_proj = batch_sae_probe_projections[i]


                main_feats_probe_proj = (
                    torch.sum(sae_act_probe_proj[main_feature_ids]).item()
                )

                potential_absorbers_mask = torch.ones(
                    sae_act_probe_proj.size(0), dtype=torch.bool, device=sae_act_probe_proj.device
                )
                ...
                top_potential_absorbers_total_probe_proj = (
                    torch.sum(top_potential_absorbers_probe_proj).item()
                )
