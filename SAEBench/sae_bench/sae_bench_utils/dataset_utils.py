...
def get_dataset_list_of_strs(
    dataset_name: str, column_name: str, min_row_chars: int, total_chars: int, subset_name=None
) -> list[str]:
    if dataset_name == "monology/pile-uncopyrighted":
        local_file_path = "datasets--monology--pile-uncopyrighted/snapshots/3be90335b66f24456a5d6659d9c8d208c0357119/test.jsonl.zst"
        dataset = load_dataset(
            "json",
            data_files=local_file_path,
            split="train"
        )
    elif not subset_name or not len(subset_name):
        try:
            import os
            if "openwebtext" in dataset_name and os.path.exists(dataset_name):
                cache_dir = os.path.dirname(dataset_name)
                dataset = load_dataset("Skylion007/openwebtext", split="train", streaming=False, cache_dir=cache_dir, trust_remote_code=True)
            else:
                dataset = load_from_disk(dataset_name)
        except:
            dataset = load_dataset(dataset_name, split="train", streaming=False)
    else:
        dataset = load_dataset(dataset_name, name=subset_name, split="train", streaming=False)
...
def load_and_tokenize_dataset(
    ...
    dataset = get_dataset_list_of_strs(dataset_name, column_name, 100, num_tokens * 5, subset_name=subset_name)
