...
def load_dictionary_learning_batch_topk_sae(
    ...
    if repo_id == 'local':
        pt_params = torch.load(filename, map_location=torch.device("cpu"))
        config_filename = filename.replace("ae.pt", "config.json")
        if 'checkpoints' in filename:
            config_filename = filename.split("checkpoints")[0] + 'config.json'
    ...
    elif config["trainer"]["trainer_class"] == "CoBatchTopKTrainer":
        sae.cfg.architecture = "co_batch_topk"
    elif config["trainer"]["trainer_class"] == "OrtTrainer":
        sae.cfg.architecture = "ort"
    elif config["trainer"]["trainer_class"] == "CoOrtTrainer":
        sae.cfg.architecture = "co_ort"
    else:
...
def load_dictionary_learning_matryoshka_batch_topk_sae(
    ...
    elif config["trainer"]["trainer_class"] == "CoMatryoshkaBatchTopKTrainer":
        sae.cfg.architecture = "co_matryoshka_batch_topk"
    else:
        raise ValueError(f"Unknown trainer class: {config['trainer']['trainer_class']}")
